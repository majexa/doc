...
class Markdown implements MarkdownInterface {
  ...
  protected function _doAnchors_inline_callback($matches) {
    ...
    if ($unhashed != $url) $url = preg_replace('/^<(.*)>$/', '\1', $unhashed);
...
abstract class _MarkdownExtra_TmpImpl extends \Michelf\Markdown {
  ...
  protected function _doAnchors_reference_callback($matches) {
    ...
    if (isset($this->urls[$link_id])) {
      ...
      if (isset($this->ref_attr[$link_id])) $result .= $this->ref_attr[$link_id];
  ...
  protected function _doTable_makeAlignAttr($alignname) {
    if (empty($this->table_align_class_tmpl)) return " align=\"$alignname\"";
  ...
  protected function _doTable_callback($matches) {
    ...
    foreach ($separators as $n => $s) {
      if (preg_match('/^ *-+: *$/', $s)) $attr[$n] = $this->_doTable_makeAlignAttr('right');
      else if (preg_match('/^ *:-+: *$/', $s)) $attr[$n] = $this->_doTable_makeAlignAttr('center');
      else if (preg_match('/^ *:-+ *$/', $s)) $attr[$n] = $this->_doTable_makeAlignAttr('left');
      else
        $attr[$n] = '';
    }
