...
class Ansi2Html {
  ...
  static function ascii2entities($string) {
    for ($i = 128; $i <= 255; $i++) {
      $entity = htmlentities(chr($i));
  ...
  static function ansi_decode($matches) {
    static $colors = [
      '#000',
      '#f00',
      '#00c000',
...
//              break;
  ...
  static function convert($str) {
    $str = htmlspecialchars($str);
    // Replace special characters to their corresponding HTML entities
    //$str = self::ascii2entities($str);
    // Replace ANSI codes.
    $str = preg_replace_callback('/(?:\e\[\d+(?:;\d+)*m)+/', 'self::ansi_decode', "$str\033[0m");
    // Strip ASCII bell.
